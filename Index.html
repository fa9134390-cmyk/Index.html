...
<body>
  ...
  <script>
    const firebaseConfig = {
      apiKey: "AIzaSyA5YF6GcqwmuZdH4EKfpVEQ6A_2HVtAmZY",
      ...
      db.collection("videos").orderBy("created","desc").onSnapshot(snapshot=>{
        ...
        snapshot.forEach(doc=>{
          ...
          card.innerHTML=`
            ...
            <p>by ${v.user}</p>
            <div class="actions">
              <button onclick="likeVideo('${doc.id}')">👍 Like (${v.likes||0})</button>
              <button onclick="subscribeChannel('${doc.id}', '${auth.currentUser?.email||''}')">🔔 Subscribe (${v.subscribers?.length||0})</button>
            </div>
          `;
